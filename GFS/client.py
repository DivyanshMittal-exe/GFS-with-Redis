...
class GFSClient:
  ...
  def get_chunk_handle(self, filename, offset):
    ...
    self.channel2.basic_publish(exchange=SERVER_REQUEST_EXCHANGE, routing_key=SERVER_REQUEST_QUEUE,
    ...
    for method_frame, properties, body in self.channel2.consume(queue=self.chunk_handle_queue.method.queue,
                                                               auto_ack=True,
  ...
  def read(self, filename, offset) -> Union[Tuple[bool, Union[bytes, str]], Tuple[bool, int, dict]]:
    ...
    if key not in self.file_offset_to_chunk_handle:
      ...
      if status != StatusCodes.CHUNK_HANDLE_REQUEST_SUCCESSFUL:
          # os.exit()
          print(f'Oh no, I have failed to get the chunk handle for {filename} at {offset}')
